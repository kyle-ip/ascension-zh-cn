# -*- coding: utf-8 -*-
"""Apply polished flavors using exact lua card ids; fix leftover UI drafts."""
...
# Exact en -> zh for remaining ui_runtime drafts
UI_EXACT: dict[str, str] = {
...
UI_PREFIX: list[tuple[str, str]] = [
    ...
    (
        ...
        "<margin-right=8em>要击败怪物，你必须拥有足够的战力。只要战力足够，你可以击败任意数量的怪物。可选中的卡牌会高亮。</margin-right>",
    ),
    (
...
def main() -> None:
    ...
    for r in rows:
        if r["id"] in FLAVORS:
            ...
            n += 1
    # report still-latin flavors
    import re

    word = re.compile(r"[A-Za-z]{3,}")
    for r in rows:
        fl = r.get("flavor_text") or ""
        if word.search(fl):
    ...
    with ZH_LUA.open("w", encoding="utf-8", newline="") as f:
        ...
        w.writeheader()
        w.writerows(rows)
    print(f"flavors_applied={n} still_latin={unmatched}")
    ...
    for r in ui:
        ...
        if en in UI_EXACT:
            ...
            un += 1
            continue
        for pref, zh in UI_PREFIX:
            if en.startswith(pref) or pref in en:
                r["zh"] = zh
                un += 1
                break
    with UI_RT.open("w", encoding="utf-8", newline="") as f:
        ...
        w.writeheader()
        w.writerows(ui)
    print(f"ui_runtime_fixed={un}")

    if UI_KEYS.is_file():
        rows_k = list(csv.reader(UI_KEYS.open(encoding="utf-8", newline="")))
        for row in rows_k:
            if len(row) >= 2 and row[0] == "Key_OnlineFailGameCenter":
                row[1] = "必须登录<br>Game Center<br>才能进行在线游戏"
        with UI_KEYS.open("w", encoding="utf-8", newline="") as f:
            csv.writer(f, lineterminator="\n").writerows(rows_k)
        print("ui_keys patched")
